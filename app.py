...
def get_p_str(input_str):
    try:
        p_contents = re.findall(r'<p>(.*?)</p>', input_str, re.DOTALL)
        p_texts = [re.sub(r'<\w+>[^<]*</\w+>', '', content).strip() for content in p_contents]
        return '\n'.join(p_texts), "SUCCESS"
    except Exception as e:
        return f"opus! some exception {e}\n{traceback.format_exc()}", "FAILED"
...
def main() -> None:
    with gr.Blocks(title="DeepLang Data test project") as demo:
        ...
        with gr.Tab("html parser"):
            raw_input = gr.Textbox(lines=1, placeholder="输入需要转换的文本(英文)", label="")
            pack_output = gr.Textbox(label="输出")
            status_output = gr.Textbox(label="状态信息")
